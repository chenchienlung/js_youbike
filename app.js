...
fetch(url)
    ...
    .then((stations) =>{
        ...
        const s1 = stations
        .filter((station) => {
            return station.ar.includes(search_input)
        })

        if (search_input.length === 0) {
            textElement.innerHTML = "<p>請輸入關鍵字</p>"
            return
        }
        ...
        s1.forEach((station) => {
            const station_name = station.sna.replace("YouBike2.0_", "")
            ...
            ${station_name} (可借車輛:${bikes}) <br> ${address} </li>`
        })
        html += "</ul>"
